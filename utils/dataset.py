...
class VOCDataset(Dataset):
    def __init__(self, root, num_images=None, crop_size=64, train=True):
        '''
        Args:
        ...
        self.train = train
        # 简单划分：前80%训练，20%测试
        split = int(0.8 * len(self.img_names))
        if train:
            self.img_names = self.img_names[:split]
        else:
            self.img_names = self.img_names[split:]
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            # 测试时可保留原尺寸，但为了统一处理，也可resize到固定大小，此处保持原尺寸
            pass
...
def get_dataloader(config, train=True):
    dataset = VOCDataset(config['data_dir'], num_images=config['data_loader']['num_images'],
                         crop_size=config['data_loader']['crop_size'], train=train)
